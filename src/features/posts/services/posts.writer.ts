...
export async function deletePost(
  ...
  if (post.status === "published") {
    ...
    const tasks = [
      invalidatePost(context, post.slug),
      invalidateSyncHash(context, data.id),
    ...
    const { invalidateSyncHash } = await import(
      "@/features/cache/cache-invalidation"
    );
    context.executionCtx.waitUntil(invalidateSyncHash(context, data.id));
  }
